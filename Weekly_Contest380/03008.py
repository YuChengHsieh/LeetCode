...
class Solution:
    def beautifulIndices(self, s: str, a: str, b: str, k: int) -> list[int]:
        ...
        ans = []
        for a_idx in a_idxs:
            idx = bisect_left(b_idxs,a_idx)
            if (idx-1 >= 0 and abs(a_idx-b_idxs[idx-1]) <= k) or \
                (idx+1 < len(b_idxs) and abs(a_idx-b_idxs[idx+1]) <= k) or\
                (idx >= 0 and idx < len(b_idxs) and abs(a_idx-b_idxs[idx]) <= k):
                    ans.append(a_idx)
        return ans
